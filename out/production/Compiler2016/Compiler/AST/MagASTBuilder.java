 ...
 */
public class MagASTBuilder extends MagBaseListener {
    MagParser parser;
    ...
    public void exitType(MagParser.TypeContext ctx) {
        switch (ctx.getText()) {
            case "int" : stack.push(new IntType()); break;
            case "string" : stack.push(new StringType()); break;
            case "bool" : stack.push(new boolType()); break;
            default: stack.push(new ClassType(Symbol.getSymbol(ctx.ID().getText())));
        }
        stack.peek().info = new Info(ctx.getStart().getLine(), ctx.getStart().getCharPositionInLine());
    }
